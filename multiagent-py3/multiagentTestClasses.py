...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.thresholdPoints = [int(s) for s in testDict.get('thresholdPoints', '').split()]
        self.maxPoints = 2 * sum(self.thresholdPoints)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        def gradeThreshold(value, minimum, thresholds, name):
            ...
            if passed:
                for t, pt in zip(thresholds, self.thresholdPoints):
                    if value >= t:
                        points += pt
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                if len(thresholds):
                    self.addMessage("%s %s (%s of %s points)" % (value, name, points, sum(self.thresholdPoints)))
                else:
                    self.addMessage("%s %s (%s of %s points)" % (value, name, points, 0))
            ...
            if minimum != None:
                ...
                for idx, threshold in enumerate(thresholds):
                    self.addMessage("    >= %s:  %s points" %
                                    (threshold, sum(self.thresholdPoints[:idx + 1])))
